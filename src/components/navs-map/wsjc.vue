<template>
    <div id="wsjc">
        ...
        <app-head id="wsjc-head"></app-head>
        <app-left id="wsjc-left" ref="leftchild"></app-left>
        <app-right id="wsjc-right" ref="rightchild"></app-right>
    </div>
</template>
<style>
    #wsjc{
...
import head from '@/components/wsjc/head'
import right from '@/components/wsjc/right'
import left from '@/components/wsjc/left'
...
export default {
    ...
    methods: {
        mappp:function(){/*BMAP_SATELLITE_MAP BMAP_HYBRID_MAP*/
            ...
            bdary.get("山西省"+this.$store.state.diqu, (rs) => { 
                ...
                for(var i=0;i<this.aaa.length;i++ ){
                    myIcon = new BMap.Icon(cccv, new BMap.Size(30,30));
        ...
        axioss:function(){
            this.$refs.leftchild.wsjcleftchart()
            this.$refs.rightchild.wsjcrightchart()
        }
    },
    mounted(){
        this.mappp()
